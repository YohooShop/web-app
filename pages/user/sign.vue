				...
				<view class="calendar">
					...
					<view class="myDateTable">
						<view v-for="(item, j) in days" :key="j" class="dateCell x-c">
							...
							<view v-else>
								<!-- 已签到日期 -->
								<view v-if="item.sign" class="cell is-sign">
									<text>{{ item.day }}</text>
								</view>
								<!-- 漏签 -->
								<view
	...
	export default {
		...
		methods: {
			...
			getSignList() {
				...
				let that = this;
				let params = {'date':query};
		
				this.$store.dispatch('SignList',params).then((result)=>{
					console.log(result);
				
				let emptyDays = that.calculateEmptyGrids();
			    that.cuntinueDays =  result.cuntinueDays
				that.days = [...emptyDays, ...result.days];
				that.selSign();
			...
			onSign() {
				let that = this;
				this.$store.dispatch('Sign').then((result)=>{
					console.log(result);
					that.getSignList();
