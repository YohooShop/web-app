						...
						<view class="canvas-list">
							<view class="canvas-item" :style="[{zIndex:index2}]" v-for="(iteml,index2) in awardsList" :key="index2">
								<view class="canvas-item-text" :style="[{transform:'rotate('+iteml.turn+')'}]">
									...
									<image mode="aspectFit" class="canvas-item-text-img" src="../../static/img/xiaolian.png" v-if="iteml.type == 3"></image>
									<image  mode="scaleToFill" class="canvas-item-text-img" src="../../static/img/youhuiquan.png" v-if="iteml.type == 2"></image>
									<!-- <image class="canvas-item-text-img" src="../../static/img/jingyan.png" v-if="iteml.type == 1"></image> -->
									<image  mode="aspectFit" class="canvas-item-text-img" src="../../static/img/jifenimg.png" v-if="iteml.type == 0"></image>
								</view>
							</view>
						</view>
					</view>
...
<script>
	import {integralLucky ,winningPrize} from '@/api/action'
	export default {
		data() {
			return {
				awardsConfig: {
		...
		methods: {
			...
			initdata:function(that){
				let params = {mold: that.mold}
				integralLucky(params).then(res=>{
						that.awardsConfig = res.data
						that.chishu = that.awardsConfig.luckdraw;
							// 获取奖品的个数
